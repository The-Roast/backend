...
class CRUDDigest(CRUDBase[Digest, DigestCreate, DigestUpdate]):
    ...
    def get_by_email(self, db: Session, *, email: str) -> Optional[Digest]:
        stmt = select(Digest).where(Digest.email == email)
        return db.execute(stmt).first()

    def create_with_owner(self, db: Session, *, obj_in: DigestCreate, user_uuid: UUID) -> Digest:
        stmt = insert(Digest).values(
            user_uuid=obj_in.user_uuid if obj_in.user_uuid else user_uuid,
            name=obj_in.name,
    ...
    def get_multi_by_owner(self, db: Session, *, user_uuid, skip: Optional[int], limit: Optional[int]):
